...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"aboutInfo"]) {
        WebViewController *infoViewController = segue.destinationViewController;
    }
}
...
- (IBAction) save: (id) sender {
    ...
    NSString *nid = @"ID";// this is the name for to find the correct record.
    NSString *ntext = NoteText.text;
    
    NSString *url = [NSString stringWithFormat:@"http://192.168.0.125:51212/firstapp/insert.php?nid=%@&ntext=%@", nid, ntext];
